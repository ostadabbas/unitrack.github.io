...
class Detector(object):
    def __init__(self, args, model=None, seq_num=1):
        self.args = args
        self.detr = model
        ...
        self.save_path = os.path.join(self.args.output_dir, f'results/MOT20-0{seq_num}')
        os.makedirs(self.save_path, exist_ok=True)

        self.predict_path = os.path.join(self.args.output_dir, args.exp_name)
        os.makedirs(self.predict_path, exist_ok=True)
        if os.path.exists(os.path.join(self.predict_path, f'MOT20-0{self.seq_num}.txt')):
            os.remove(os.path.join(self.predict_path, f'MOT20-0{self.seq_num}.txt'))

    # [Keep all other methods the same]

    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, 'MOT20/train')
        result_filename = os.path.join(self.predict_path, 'gt.txt')
        evaluator = Evaluator(data_root, f'MOT20-0{self.seq_num}')
...
if __name__ == '__main__':
    ...
    # for MOT20 submit
    sub_dir = 'MOT20/train'
    seq_nums = ['01', '02', '03', '05']  # MOT20 test sequences

    for seq_num in seq_nums:
        det = Detector(args, model=detr, seq_num=seq_num)
        det.detect()

    print('Results saved for MOT20 test sequences')

    # Evaluation on MOT20 train set
    sub_dir = 'MOT20/train'
    seq_nums = os.listdir('/data/Dataset/mot/MOT20/train')
    accs = []
    seqs = []
    for seq_num in seq_nums:
