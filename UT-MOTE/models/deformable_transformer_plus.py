...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        if two_stage:
            self.query_embed = nn.Embedding(self.num_queries, d_model * 2)  # Ensure correct dimension
            self.tgt_embed = nn.Embedding(self.num_queries, d_model)  # For target embeddings if needed

            # Adjust the pos_trans to output d_model * 2 if it is transforming query embeddings
            self.pos_trans = nn.Linear(d_model * 2, d_model * 2)  # Adjust input dimension if needed
            self.pos_trans_norm = nn.LayerNorm(d_model * 2)

            self.enc_output = nn.Linear(d_model * 2,d_model *2)  # Ensure enc_output matches expected sizes
            self.enc_output_norm = nn.LayerNorm(d_model)
        else:
            self.reference_points = nn.Linear(d_model, 2)  # This is fine
