...
def visualize_features(tensor):
    ...
    save_dir = "/work/aclab/bishoy/results/splatted_features_mot17"
...
def viz_flow(img, flo):
    # Assuming `flo` and `img` are PyTorch tensors.
...
class ClipMatcher(SetCriterion):
    ...
    def loss_boxes(self, outputs, gt_instances: List[Instances], indices: List[tuple], num_boxes):
        ...
        # Process splatted features to extract occlusion data
        # Correcting the extraction of splatted_features for occlusion
#         splatted_features = splatted_features[idx]  # Indexing to get relevant features
#         occlusion_weights = 1 / (1 + splatted_features.exp())
#         occlusion_weights = occlusion_weights.mean(dim=[1, 2])  # Averaging over spatial dimensions (height and width)
...
class SoftmaxSplattingModule(nn.Module):
    def __init__(self, num_feature_levels):
        ...
        # Add projection layers to handle different channel dimensions
        self.projections = nn.ModuleList()
        # The actual channel dimensions from the backbone output
        channels = [512, 1024, 2048]  # Exact dimensions from debug output
        for i in range(min(num_feature_levels, len(channels))):
            self.projections.append(nn.Conv2d(channels[i], 256, kernel_size=1))

    def forward(self, features: List[torch.Tensor], optical_flow: torch.Tensor) -> torch.Tensor:
        # Normalize weights across feature levels
        weights = F.softmax(self.weights, dim=0)
        weights = weights.view(-1, 1, 1, 1)

        # Determine the size to interpolate to (smallest feature map dimensions)
        min_height = min(f.size(2) for f in features)
        min_width = min(f.size(3) for f in features)

        # Project and resize all feature maps to the same channel dimension and spatial size
        projected_features = []
        for i, f in enumerate(features):
            if i < len(self.projections):
                # Apply projection to standardize channel dimension
                projected = self.projections[i](f)
                # Resize to the common spatial dimensions
                resized = F.interpolate(projected, size=(min_height, min_width), mode='bilinear', align_corners=False)
                projected_features.append(resized)

        # Ensure optical_flow is a tensor and has the correct dimensions
        ...
        # Create a disocclusion map from optical flow
        # Compute the divergence of the flow field
        if optical_flow_resized.size(1) == 2:  # Assuming 2 channel flow [B, 2, H, W]
            # Extract flow components
            flow_x = optical_flow_resized[:, 0:1, :, :]
            flow_y = optical_flow_resized[:, 1:2, :, :]
            
            # Compute spatial gradients of the flow
            # For x-component: gradient in x direction
            flow_x_dx = torch.zeros_like(flow_x)
            flow_x_dx[:, :, :, 1:-1] = (flow_x[:, :, :, 2:] - flow_x[:, :, :, :-2]) / 2.0
            
            # For y-component: gradient in y direction
            flow_y_dy = torch.zeros_like(flow_y)
            flow_y_dy[:, :, 1:-1, :] = (flow_y[:, :, 2:, :] - flow_y[:, :, :-2, :]) / 2.0
            
            # Compute divergence: div = df_x/dx + df_y/dy
            divergence = flow_x_dx + flow_y_dy
            
            # Negative divergence indicates disocclusion (expansion)
            disocclusion_map = -divergence
            
            # Normalize to [0, 1] for visualization
            disocclusion_map = (disocclusion_map - disocclusion_map.min()) / (disocclusion_map.max() - disocclusion_map.min() + 1e-6)
            
            # Expand to match feature channels
            disocclusion_expanded = disocclusion_map.repeat(1, 256, 1, 1)
        else:
            # Create a default map if flow doesn't have the expected shape
            disocclusion_expanded = torch.zeros((optical_flow_resized.size(0), 256, min_height, min_width))

        # Apply weights to features and disocclusion map
        weighted_features = [w * f for w, f in zip(weights[:len(projected_features)], projected_features)]
        weighted_disocclusion = weights[-1] * disocclusion_expanded

        # Combine features and weighted disocclusion map
        all_features = weighted_features + [weighted_disocclusion]
...
class SimpleOpticalFlowModuleOpenCV(nn.Module):
    ...
    def forward(self, frame1, frame2):
        # Convert PyTorch tensors to CPU tensors
        frame1_np = frame1#.to('cpu')
        frame2_np = frame2#.to('cpu')

        # Standardize the batch dimension handling
        ...
        # Ensure the frames are in the correct format for OpenCV
        ...
        # Convert flow to a PyTorch tensor
        flow_tensor = torch.from_numpy(flow).permute(2, 0, 1).float()  # Convert to [C, H, W] format
        return flow_tensor
...
class SimpleOpticalFlowModule(nn.Module):
    def __init__(self, device='cpu'):
        ...
        # Create a proper argparse namespace for RAFT initialization
        import argparse
        ...
        # Load RAFT model weights - handle DataParallel format
        model_weights = torch.load('/home/nail/Documents/MOTR/RAFT/models/raft-things.pth', map_location=self.device)
        
        # The model weights were saved with DataParallel, so they have 'module.' prefix
    ...
    def forward(self, img1, img2):
        """Use RAFT for optical flow calculation"""
        # Handle batch dimension
        if img1.dim() == 4 and img1.size(0) == 1:  # If batch dimension is present
            img1 = img1.squeeze(0)
        if img2.dim() == 4 and img2.size(0) == 1:
            img2 = img2.squeeze(0)
        
        # RAFT expects normalized images in the range [-1, 1]
        # Our images are already normalized to [0, 1], so we need to scale them to [-1, 1]
        img1_scaled = 2 * img1 - 1.0
        img2_scaled = 2 * img2 - 1.0
        
        # Add batch dimension if needed
        if img1_scaled.dim() == 3:
            img1_scaled = img1_scaled.unsqueeze(0)
        if img2_scaled.dim() == 3:
            img2_scaled = img2_scaled.unsqueeze(0)
        
        # RAFT processes images at their original resolution
        with torch.no_grad():
            # RAFT returns a list of flow predictions at different iterations
            # We use the final prediction (index -1)
            flow_predictions = self.model(img1_scaled, img2_scaled, iters=12)
...
class MOTR(nn.Module):
    ...
    def _generate_empty_tracks(self):
        logging.debug("Generating empty tracks for MOTR.")
        ...
        # Attempting to generate reference points
        try:
    ...
    def _forward_single_image(self, samples, optical_flow, track_instances: Instances):
        ...
        # Ensure optical_flow is on the correct device
        ...
        if isinstance(optical_flow, NestedTensor):
            optical_flow = optical_flow.tensors  # Extract the tensor component
    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last):
        ...
        if self.memory_bank is not None:
            ...
            if self.training:
                self.criterion.calc_loss_for_track_scores(track_instances, frame_res['splatted_features'])
        tmp = {}
        tmp['init_track_instances'] = self._generate_empty_tracks()
        tmp['splatted_features'] = frame_res['splatted_features']
        tmp['track_instances'] = track_instances
        if not is_last:
            logging.debug('Is Last Query Embed')
    ...
    def inference_single_image(self, img, ori_img_size, optical_flow=None, track_instances=None):
        ...
        logging.debug("_forward_single_image")
    ...
    def forward(self, data: dict):
        ...
        for frame_index, frame in enumerate(frames):
            ...
            logging.debug(f"Processing frame {frame_index}, is last: {is_last}")
            # exit()
            if frame_index > 0:
                try:
                    logging.debug(f"Computing optical flow between previous frame and current frame.")
                    optical_flow = self.optical_flow_module(previous_frame_tensors, frame)
                    logging.debug(f"Optical Flow Model Sucess {optical_flow.shape}")
                    if optical_flow is None:
            ...
            logging.debug(f"Optical flow shape: {optical_flow.shape}, dtype: {optical_flow.dtype}, device: {optical_flow.device}")
