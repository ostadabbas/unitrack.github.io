...
class MOTE(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
        ...
        if two_stage:
            self.query_embed = nn.Embedding(num_queries, d_model * 2)  # align with expected input dimensions for pos_trans
            self.tgt_embed = nn.Embedding(num_queries, d_model)
            self.pos_trans = nn.Linear(d_model * 2, d_model * 2)  # Ensure alignment if transforming query embeddings
            self.pos_trans_norm = nn.LayerNorm(d_model * 2)
            self.enc_output = nn.Linear(d_model * 2, d_model * 2)  # Adjust to the correct output feature size
            self.enc_output_norm = nn.LayerNorm(d_model * 2)  # Align normalization layer dimensions
        else:
            self.reference_points = nn.Linear(d_model, 2)  # For single-stage operation
    ...
    def _generate_empty_tracks(self):
        track_instances = Instances((self.num_queries,))
        num_queries, dim = self.query_embed.weight.shape
        device = self.query_embed.weight.device
        print("Query Embed Weight Shape:", self.query_embed.weight.shape)

        if self.two_stage:
            print("Running in two-stage mode.")
            query_embed_output = self.query_embed.weight  # This should now be [300 x 256]
            print("Query Embed Output Shape:", query_embed_output.shape)
            transformed_query = self.pos_trans(query_embed_output)  # Transform if necessary
            print("Transformed Query Shape:", transformed_query.shape)
            reference_points = self.transformer.enc_output(self.enc_output_norm(transformed_query))
            print("Reference Points Shape:", reference_points.shape)
        else:
            reference_points = self.transformer.reference_points(query_embed_output[:, :dim // 2])
    ...
    def _forward_single_image(self, samples, track_instances: Instances):
        ...
        if not isinstance(samples, NestedTensor):
            samples = NestedTensor(samples, None)
        print("After", samples)
        ...
        print("enc_outputs_coord_unact", enc_outputs_coord_unact)
        exit()
        outputs_classes = []
        outputs_coords = []
            ...
            for frame_index, frame in enumerate(frames):
                ...
                if self.use_checkpoint and frame_index < len(frames) - 2:
                    def fn(frame, *args):
                        ...
                        tmp = Instances((frame.shape[1],))
                        tmp.query_pos = args[0]
                        tmp.ref_pts = args[1]
                        out = self._forward_single_image(frame, track_instances=tmp)
                        return out
                    frame_res = checkpoint.checkpoint(fn, frame, track_instances.query_pos, track_instances.ref_pts)
                else:
                    frame_res = self._forward_single_image(frame, track_instances=track_instances)
                frame_res = self._post_process_single_image(frame_res, track_instances, is_last)
                outputs['pred_logits'].append(frame_res['pred_logits'])
                outputs['pred_boxes'].append(frame_res['pred_boxes'])
                if not self.training:
                    track_instances = frame_res['track_instances']
                if self.memory_bank is not None and frame_index < len(frames) - 1:
                    outputs['hs'].append(frame_res['hs'])
            if self.training:
                outputs['track_instances'] = frame_res['track_instances']
                outputs = self.criterion(outputs)
            else:
                outputs = self.post_process(outputs, data['ori_img_size'])
            return outputs
