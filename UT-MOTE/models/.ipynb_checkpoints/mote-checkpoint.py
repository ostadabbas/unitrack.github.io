...
from .etem import build as build_query_interaction_layer
...
def visualize_features(tensor):
    ...
    plt.colorbar()
    plt.title("Visualized Splatted Feature")
    plt.savefig("visualization_plots/combined_plot.png")
...
class SoftmaxSplattingModule(nn.Module):
    ...
    def forward(self, features: List[torch.Tensor], optical_flow: torch.Tensor) -> torch.Tensor:
        ...
        # Interpolate all feature maps to the smallest size
        ...
        # Check if the optical flow needs to be expanded to match the number of channels
        if optical_flow_resized.size(1) == 2:  # Assuming 2 channel flow
            optical_flow_expanded = optical_flow_resized.repeat(1, resized_features[0].size(1) // 2, 1, 1)
        else:
            # Create a default flow that does nothing if conditions are not met
            optical_flow_expanded = torch.zeros_like(resized_features[0])

        # Apply the last weight to the expanded optical flow
        weighted_flow = weights[-1] * optical_flow_expanded

        # Apply the weights to each resized feature map
        weighted_features = [w * f for w, f in zip(weights, resized_features)]

        # Combine features and weighted optical flow
...
#         visualize_features(splatted_features)
    ...
    def forward(self, data: dict):
        ...
        for frame_index, frame in enumerate(frames):
            ...
            if frame_index > 0:
                logging.debug(f"Image 1 Shape{previous_frame_tensors.shape}")
                logging.debug(f"Image 2 Shape{frame.shape}")
                try:
                ...
                logging.debug(f"Optical flow shape {optical_flow.shape}")
                logging.debug(f"Optical flow computed for frame {frame_index}")
#                 viz_flow(previous_frame_tensors, optical_flow)

            else:
                # Initialize a dummy optical flow with zeros for the first frame
                optical_flow = torch.zeros((frame.size(0), 2, *frame.shape[2:]), device=frame.device)
