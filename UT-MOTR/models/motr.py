...
class ClipMatcher(SetCriterion):
    ...
    def loss_unitrack(self, outputs, gt_instances, indices, num_boxes, **kwargs):
        """Compute Unitrack loss."""
        if not hasattr(outputs, 'track_instances') or not hasattr(outputs['track_instances'], 'obj_idxes'):
            return {'loss_unitrack': torch.tensor(0.0, device=self.sample_device)}

        unitrack_outputs = {
            'pred_boxes': outputs['track_instances'].pred_boxes.unsqueeze(0),
            'pred_logits': outputs['track_instances'].pred_logits.unsqueeze(0),
            'track_ids': outputs['track_instances'].obj_idxes.unsqueeze(0)
        }
        
        unitrack_targets = [{
            'boxes': gt_inst.boxes,
            'labels': gt_inst.labels,
            'track_ids': gt_inst.obj_ids if hasattr(gt_inst, 'obj_ids') else gt_inst.track_ids
        } for gt_inst in gt_instances]
        
        unitrack_losses = self.unitrack(unitrack_outputs, unitrack_targets)
        return {k: v * self.weight_dict.get(f'loss_{k}', 1.0) for k, v in unitrack_losses.items()}
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        # step9. calculate Unitrack loss after matching
        if active_idxes.any():
            unitrack_outputs = {
                'pred_boxes': active_track_boxes.unsqueeze(0),
                'pred_logits': track_instances.pred_logits[active_idxes].unsqueeze(0),
                'track_ids': track_instances.obj_idxes[active_idxes].unsqueeze(0)
            }
            
            unitrack_targets = [{
                'boxes': gt_boxes,
                'labels': gt_instances_i.labels[track_instances.matched_gt_idxes[active_idxes]],
                'track_ids': gt_instances_i.obj_ids[track_instances.matched_gt_idxes[active_idxes]]
            }]
            
            unitrack_losses = self.unitrack(unitrack_outputs, unitrack_targets)
            for k, v in unitrack_losses.items():
                self.losses_dict[f'frame_{self._current_frame_idx}_loss_{k}'] = v * self.weight_dict.get('loss_unitrack', 1.0)
        else:
            # Add zero Unitrack loss when no active tracks
            self.losses_dict[f'frame_{self._current_frame_idx}_loss_unitrack'] = torch.tensor(0.0, device=pred_logits_i.device)
    ...
    def forward(self, outputs, input_data: dict):
        ...
        # Add Unitrack loss if track_instances are available (Option 1: Improved logging)
        if 'track_instances' in outputs and outputs['track_instances'] is not None and hasattr(outputs['track_instances'], 'obj_idxes'):
            unitrack_outputs = {
                'pred_boxes': outputs['track_instances'].pred_boxes.unsqueeze(0),
                'pred_logits': outputs['track_instances'].pred_logits.unsqueeze(0),
                'track_ids': outputs['track_instances'].obj_idxes.unsqueeze(0)
            ...
            # Prepare targets for Unitrack
            unitrack_targets = [{
                'boxes': input_data['boxes'][0],
                'labels': input_data['labels'][0],
                'track_ids': input_data['track_ids'][0] if 'track_ids' in input_data else input_data['obj_ids'][0]
            }]
            
            unitrack_losses = self.unitrack(unitrack_outputs, unitrack_targets)
            
            # Add main Unitrack loss
            if 'loss_unitrack' in unitrack_losses:
                losses['frame_0_loss_unitrack'] = unitrack_losses['loss_unitrack'] * self.weight_dict.get('loss_unitrack', 1.0)
            
            # Add batch-specific Unitrack losses if present
            for k, v in unitrack_losses.items():
                if k.startswith('batch_'):
                    losses[f'frame_0_{k}'] = v * self.weight_dict.get('loss_unitrack', 1.0)
                    
        elif 'track_instances' not in outputs or outputs['track_instances'] is None:
            # This is expected for last frames - MOTR doesn't compute track embeddings for last frame
            # No Unitrack loss needed (this is correct behavior)
            pass
        else:
            # Unexpected case - track_instances exists but missing required attributes
            print("WARNING: track_instances exists but missing obj_idxes attribute")
...
def build(args):
    ...
    # Create base weight dict
    base_weight_dict = {
        'loss_ce': args.cls_loss_coef,
        'loss_bbox': args.bbox_loss_coef,
        'loss_giou': args.giou_loss_coef,
        'loss_unitrack': getattr(args, 'unitrack_loss_coef', 1.0)
    }
    
    # Create frame-specific weights
    weight_dict = {}
    for i in range(num_frames_per_batch):
        weight_dict.update({
            f"frame_{i}_loss_ce": base_weight_dict['loss_ce'],
            f'frame_{i}_loss_bbox': base_weight_dict['loss_bbox'],
            f'frame_{i}_loss_giou': base_weight_dict['loss_giou'],
            f'frame_{i}_loss_unitrack': base_weight_dict['loss_unitrack']
        })
        ...
        for i in range(num_frames_per_batch):
            for j in range(args.dec_layers - 1):
                aux_weight_dict.update({
                    ...
                    f'frame_{i}_aux{j}_loss_unitrack': base_weight_dict['loss_unitrack']
                })
    ...
    if args.memory_bank_type is not None and len(args.memory_bank_type) > 0:
        ...
        memory_bank = None
    losses = ['labels', 'boxes']
    criterion = ClipMatcher(num_classes=1,
                          matcher=img_matcher,
