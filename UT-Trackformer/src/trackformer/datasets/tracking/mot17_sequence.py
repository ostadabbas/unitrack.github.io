...
class MOT17Sequence(Dataset):
    ...
    def __init__(self, root_dir: str = 'data', seq_name: Optional[str] = None,
        ...
        # self._data_dir = osp.join(root_dir, 'MOT17')

        # for dancetrack
        # self._data_dir = "PROJECT_ROOT/datasets/mot/MOT17/"
        # for sportmot
        self._data_dir = "DATASET_ROOT/sportsmot_publish/dataset/"
    ...
    def _sequence(self) -> List[dict]:
        ...
        total = [
            {'gt': boxes[i],
             'im_path': osp.join(img_dir, f"{i:06d}.jpg"), # 06 for dance
             'vis': visibility[i],
             'dets': dets[i]}
