...
class DetMOTDetection:
    def __init__(self, args, data_txt_path: str, seqs_folder, transform):
        ...
        self.vis = args.vis  # 没有使用
        self.video_dict = {}
        self.mot_path = args.mot_path

        # with open(os.path.join(self.mot_path, data_txt_path), 'r') as file:
    ...
    def _pre_single_frame(self, vid, idx: int):
        ...
        # Determine dataset name based on the video path
        if 'MOT20' in vid:
            dataset_name = 'MOT20'
        else:
            dataset_name = 'MOT17'
...
def build(image_set, args):
    root = Path(os.path.join(args.mot_path, 'MOT'))
    assert root.exists(), f'provided MOT path {root} does not exist'
    transform = build_transform(args, image_set)
    
    # Determine which dataset to use based on use_mot20 flag
    if hasattr(args, 'use_mot20') and args.use_mot20:
        dataset_name = 'MOT20'
        if image_set == 'train':
            data_txt_path = 'MOT/MOT20/train'
        elif image_set == 'val':
            data_txt_path = 'MOT/MOT20/test'  # MOT20 uses test set for validation
    else:
        dataset_name = 'MOT17'
        if image_set == 'train':
            data_txt_path = 'MOT/MOT17_all/train'
        elif image_set == 'val':
            data_txt_path = 'MOT/MOT17/test'
    ...
    # Set the dataset name for proper labeling
