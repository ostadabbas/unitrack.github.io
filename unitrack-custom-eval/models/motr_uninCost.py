...
class ClipMatcher(SetCriterion):
    def __init__(self, num_classes,
        ...
        self.g_size = g_size
        # Store the flag to enable/disable Unitrack loss
        self.enable_unitrack_loss = enable_unitrack_loss
        # Initialize the Unitrack criterion for tracking loss only if enabled
        if self.enable_unitrack_loss:
    ...
    def loss_boxes(self, outputs, gt_instances: List[Instances], indices: List[tuple], num_boxes):
        ...
        # for pad target, don't calculate regression loss, judged by whether obj_idxes=-1
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        self.num_samples += len(gt_instances_i)*self.g_size + num_disappear_track
        ...
        if 'aux_outputs' in outputs: # 此处匹配时对新生儿时重新算对应关系的，不直接使用最后一层box输出的对应关系
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                # new_matched_indices_layer = match_for_single_decoder_layer(unmatched_outputs_layer, self.matcher, untracked_gt_instances, unmatched_track_idxes, untracked_tgt_indexes)
    ...
    def calculate_unitrack_loss(self, all_outputs, all_gt_instances):
        """Calculate Unitrack tracking loss across a sequence of frames.
        
        This method is called at the end of clip processing when all track IDs have been assigned.
        It's specifically designed to operate on a sequence of frames rather than individual frames.
        
        Args:
            all_outputs: List of output dictionaries for each frame in the sequence
            all_gt_instances: List of ground truth instances for each frame in the sequence
            
        Returns:
            Dictionary of Unitrack losses
        """
        if len(all_outputs) <= 1:
            # Need at least 2 frames for tracking loss
            return {'loss_unitrack': torch.tensor(0.0, device=all_outputs[0]['pred_boxes'].device)}
            
        device = all_outputs[-1]['pred_boxes'].device
        dtype = all_outputs[-1]['pred_boxes'].dtype

        # Aggregate predictions (boxes / logits) and track IDs from
        # *all* frames so that spatial & temporal consistency terms
        # have multiple observations per track.
        pred_boxes_list   = []
        pred_logits_list  = []
        track_ids_list    = []
        frame_indices     = []  # NEW: Track which frame each box comes from

        for f_idx, out in enumerate(all_outputs):
            if out.get('track_instances', None) is None:
                # Skip frames where we purposely dropped track_instances
                continue

            current_boxes = out['pred_boxes'][0]   # (num_q,4)
            current_logits = out['pred_logits'][0] # (num_q,C)
            
            pred_boxes_list.append(current_boxes)
            pred_logits_list.append(current_logits)

            # Prefer obj_idxes from track_instances when present
            if hasattr(out['track_instances'], 'obj_idxes'):
                cur_ids = out['track_instances'].obj_idxes.to(dtype)
            else:
                num_q = out['pred_boxes'].shape[1]
                cur_ids = torch.arange(num_q, device=device, dtype=dtype)

            # Make sure length aligns with prediction boxes of this frame
            len_pred = current_boxes.shape[0]
            if cur_ids.numel() != len_pred:
                if cur_ids.numel() < len_pred:
                    # pad with -1 for unmatched queries
                    pad_len = len_pred - cur_ids.numel()
                    cur_ids = torch.cat([cur_ids, torch.full((pad_len,), -1, dtype=dtype, device=device)])
                else:
                    cur_ids = cur_ids[:len_pred]
            
            track_ids_list.append(cur_ids)
            
            # NEW: Record frame index for each box in this frame
            frame_indices.extend([f_idx] * len_pred)

        if len(pred_boxes_list) == 0:
            # Should not happen, but be safe
            return {'loss_unitrack': torch.tensor(0.0, device=device)}

        pred_boxes = torch.cat(pred_boxes_list, dim=0).unsqueeze(0)   # (1,N,4)
        pred_logits = torch.cat(pred_logits_list, dim=0).unsqueeze(0) # (1,N,C)
        track_ids   = torch.cat(track_ids_list,   dim=0).unsqueeze(0) # (1,N)
        frame_indices = torch.tensor(frame_indices, device=device, dtype=torch.long)  # (N,)

        # IMPROVED: Aggregate GT from ALL frames for better temporal tracking context
        all_gt_boxes = []
        all_gt_labels = []  
        all_gt_track_ids = []
        gt_frame_info = []  # Track which frame each GT belongs to
        
        for frame_idx, gt in enumerate(all_gt_instances):
            if len(gt) > 0:
                all_gt_boxes.append(gt.boxes)
                
                # Handle different label attribute names
                if hasattr(gt, 'gt_classes'):
                    labels = gt.gt_classes
                elif hasattr(gt, 'labels'):
                    labels = gt.labels
                else:
                    labels = torch.zeros(len(gt.boxes), dtype=torch.long, device=device)
                all_gt_labels.append(labels)
                
                # Handle track IDs
                if hasattr(gt, 'obj_ids'):
                    track_ids_gt = gt.obj_ids.to(dtype)
                else:
                    # Fallback: create synthetic track IDs if not available
                    track_ids_gt = torch.arange(len(gt.boxes), device=device, dtype=dtype) + frame_idx * 1000
                all_gt_track_ids.append(track_ids_gt)
                
                # Track frame information for each GT object
                gt_frame_info.extend([frame_idx] * len(gt.boxes))

        if len(all_gt_boxes) == 0:
            # No GT in any frame - return zero losses
            return {'loss_unitrack': torch.tensor(0.0, device=device)}

        # Concatenate all GT data across frames
        gt_boxes = torch.cat(all_gt_boxes, dim=0)
        gt_labels = torch.cat(all_gt_labels, dim=0)  
        gt_track_ids = torch.cat(all_gt_track_ids, dim=0)
        gt_frame_info = torch.tensor(gt_frame_info, device=device, dtype=torch.long)
        
        # Create comprehensive targets with multi-frame context
        targets = [{
            'boxes': gt_boxes,
            'labels': gt_labels,
            'track_ids': gt_track_ids,
            'frame_indices': gt_frame_info  # Frame information for temporal relationships
        }]
        
        # Debug logging for multi-frame GT info
        # if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
        #     print(f"[DEBUG] Multi-frame GT: {len(gt_boxes)} total objects across {len(all_gt_instances)} frames")
        # elif not torch.distributed.is_initialized():
        #     print(f"[DEBUG] Multi-frame GT: {len(gt_boxes)} total objects across {len(all_gt_instances)} frames")

        # Keep ONLY predictions with non-negative track IDs
        track_ids_flat = track_ids.view(-1)
        pos_mask = track_ids_flat >= 0
        
        if pos_mask.sum() == 0:
            # nothing valid – l out
            return {'loss_unitrack': torch.tensor(0.0, device=device)}

        # Filter tensors - keep them 2D for now
        pred_boxes_filtered = pred_boxes.view(-1, 4)[pos_mask]  # [N, 4]
        pred_logits_filtered = pred_logits.view(-1, pred_logits.shape[-1])[pos_mask]  # [N, C]
        track_ids_filtered = track_ids_flat[pos_mask]  # [N]
        frame_indices_filtered = frame_indices[pos_mask]  # NEW: [N] - frame index for each box

        # IMPROVED SORTING: Group by track ID AND sort by frame index within each track
        if len(track_ids_filtered) > 0:
            # Create a list to store sorted tensors for each track
            sorted_pred_boxes = []
            sorted_pred_logits = []
            sorted_track_ids = []
            
            unique_track_ids = torch.unique(track_ids_filtered)
            
            for track_id in unique_track_ids:
                if track_id >= 0:  # Only process valid track IDs
                    track_mask = (track_ids_filtered == track_id)
                    track_indices = torch.where(track_mask)[0]
                    
                    # Safety check: ensure indices are within bounds
                    max_idx = track_indices.max().item() if len(track_indices) > 0 else -1
                    if max_idx >= len(pred_boxes_filtered):
                        print(f"[ERROR] Index out of bounds: max_idx={max_idx}, pred_boxes_filtered.shape[0]={len(pred_boxes_filtered)}")
                        continue
                        
                    # Extract boxes, logits, IDs, and frame indices for this track
                    if len(track_indices) > 0:
                        # NEW: Get frame indices for this track and sort by temporal order
                        track_frame_indices = frame_indices_filtered[track_indices]
                        
                        # Sort track_indices by frame order to ensure chronological ordering
                        temporal_sort_order = track_frame_indices.argsort()
                        track_indices_sorted = track_indices[temporal_sort_order]
                        
                        # Extract in temporal order
                        track_boxes = pred_boxes_filtered[track_indices_sorted]
                        track_logits = pred_logits_filtered[track_indices_sorted] 
                        track_ids_tensor = track_ids_filtered[track_indices_sorted]
                        
                        sorted_pred_boxes.append(track_boxes)
                        sorted_pred_logits.append(track_logits)
                        sorted_track_ids.append(track_ids_tensor)
            
            # Concatenate all sorted tracks
            if len(sorted_pred_boxes) > 0:
                pred_boxes_final = torch.cat(sorted_pred_boxes, dim=0)  # [N, 4]
                pred_logits_final = torch.cat(sorted_pred_logits, dim=0)  # [N, C]
                track_ids_final = torch.cat(sorted_track_ids, dim=0)  # [N]
                
                # Debug logging for temporal ordering
                # if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
                #     print(f"[DEBUG] Temporal ordering applied to {len(unique_track_ids)} tracks")
                # elif not torch.distributed.is_initialized():
                #     print(f"[DEBUG] Temporal ordering applied to {len(unique_track_ids)} tracks")
            else:
                # Warning logging for no valid tracks
                if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
                    print("No valid tracks found after sorting")
                elif not torch.distributed.is_initialized():
                    print("No valid tracks found after sorting")
                return {'loss_unitrack': torch.tensor(0.0, device=device)}
        else:
            pred_boxes_final = pred_boxes_filtered
            pred_logits_final = pred_logits_filtered
            track_ids_final = track_ids_filtered

        # Convert boxes to XYXY format after sorting
        pred_boxes_xyxy = box_ops.box_cxcywh_to_xyxy(pred_boxes_final)  # [N, 4]

        # Final safety check
        if pred_boxes_xyxy.shape[0] != pred_logits_final.shape[0] or pred_boxes_xyxy.shape[0] != track_ids_final.shape[0]:
            # Error logging for shape mismatch
            if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
                print(f"Shape mismatch after processing: pred_boxes_xyxy: {pred_boxes_xyxy.shape}, "
                           f"pred_logits_final: {pred_logits_final.shape}, track_ids_final: {track_ids_final.shape}")
            elif not torch.distributed.is_initialized():
                print(f"Shape mismatch after processing: pred_boxes_xyxy: {pred_boxes_xyxy.shape}, "
                           f"pred_logits_final: {pred_logits_final.shape}, track_ids_final: {track_ids_final.shape}")
            return {'loss_unitrack': torch.tensor(0.0, device=device)}

        # Add batch dimension for Unitrackrion - expects [batch_size, N, 4]
        outputs_for_unitrack = {
            'pred_boxes': pred_boxes_xyxy.unsqueeze(0),      # [1, N, 4]
            'pred_logits': pred_logits_final.unsqueeze(0),   # [1, N, C]  
            'track_ids': track_ids_final.unsqueeze(0)        # [1, N]
        }

        # Compute Unitrack loss
        try:
            unitrack_losses = self.unitrack_criterion(outputs_for_unitrack, targets)
        except Exception as e:
            # Error logging for Unitrack criterion failure
            if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
                print(f"Unitrackrion failed: {e}")
                import traceback
        ...
        # Debug logging for loss components (only in debug mode)
        # if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
            # print(f'[DEBUG] Unitrack loss components - spatial: {unitrack_losses["loss_unitrack_spatial"].item():.8f}, '
                        # f'temporal: {unitrack_losses["loss_unitrack_temporal"].item():.8f}')
        # elif not torch.distributed.is_initialized():
            # print(f'[DEBUG] Unitrack loss components - spatial: {unitrack_losses["loss_unitrack_spatial"].item():.8f}, '
                        # f'temporal: {unitrack_losses["loss_unitrack_temporal"].item():.8f}')

        return unitrack_losses
    ...
    def forward(self, outputs, input_data: dict):
        ...
        # Add Unitrack tracking loss across the entire clip if enabled
        if self.enable_unitrack_loss and hasattr(self, 'unitrack_criterion') and hasattr(self, 'gt_instances') and len(self.gt_instances) > 0:
            # Get all outputs and all GT instances for the clip
            all_outputs = []
            if 'all_frame_outputs' in outputs:
                all_outputs = outputs.pop('all_frame_outputs')
            
            # Calculate Unitrack loss on the sequence
            unitrack_losses = self.calculate_unitrack_loss(all_outputs, self.gt_instances)
            
            # Add Unitrack losses to the existing losses dictionary - with safety check
            if unitrack_losses is not None:
                for k, v in unitrack_losses.items():
                    losses[k] = v
            else:
                # Warning logging for None Unitrack loss
                if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
                    print("Unitrack loss calculation returned None")
                elif not torch.distributed.is_initialized():
                    print("Unitrack loss calculation returned None")
...
class RuntimeTrackerBase(object):  # 实际为一个跟踪ID分配器
    ...
    def update(self, track_instances: Instances, g_size=1):
        ...
        new_obj = (track_instances.obj_idxes == -1) & (active_idxes) # 挑选新生儿，obj_idxes=-1表示为检测query
        disappeared_obj = (track_instances.obj_idxes >= 0) & (~active_idxes)  # 跟踪中假如置信度偏低则 disappear_time++
        num_new_objs = new_obj.sum().item() // g_size

        track_instances.obj_idxes[new_obj] = (self.max_obj_id + torch.arange(num_new_objs, device=device)).view(-1, 1).repeat(1, g_size).view(-1)  # 分配ID
...
class TrackerPostProcess(nn.Module):
    ...
    def forward(self, track_instances: Instances, target_size) -> Instances:
        ...
        if len(prob):
            num_query, cls_num = prob.shape
            scores, labels = prob.reshape(-1, self.g_size, cls_num).max(1)[0].reshape(-1, 1, cls_num).repeat(1, self.g_size, 1).reshape(-1, cls_num).max(-1)
        else:
            scores = out_logits[..., 0].sigmoid()
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
                 aux_loss=True, with_box_refine=False, two_stage=False, memory_bank=None, use_checkpoint=False, query_denoise=0, g_size=1, args=None):
        ...
        self.post_process = TrackerPostProcess(g_size=g_size)
    ...
    def _generate_empty_tracks(self, proposals=None, g_size=1):
        ...
        if proposals is None:
            ...
            track_instances.ref_pts = torch.cat([self.position.weight, proposals[:, :4]]).view(-1, 1, 4).repeat(1, g_size, 1).view(-1, 4) + self.position_offset.weight # 其实是归一化的box，有这个可以得倒position query
            track_instances.query_pos = torch.cat([self.query_embed.weight, pos2posemb(proposals[:, 4:], d_model) + self.yolox_embed.weight]).view(-1, 1, d_model).repeat(1, g_size, 1).view(-1, d_model)  +  self.query_embed_offset.weight# decode的query
        ...
        track_instances.group_ids = torch.arange(g_size, dtype=torch.long, device=device).repeat(num_queries)
    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last):
        ...
        if not is_last: # 经过这步后将仅保留有ID的目标，且更新了track的query和pos
            ...
            # Preserve the track_instances for the last frame so that downstream
            # components (e.g., Unitrack loss computation) can still access the
            # assigned track IDs.
            frame_res['track_instances'] = track_instances
        return frame_res
    ...
    def inference_single_image(self, img, ori_img_size, track_instances=None, proposals=None): 
        if not isinstance(img, NestedTensor):
            img = nested_tensor_from_tensor_list(img, size_divisibility=32)  # 补pad并或者pad的mask
    ...
    def forward(self, data: dict):
        ...
        keys = list(self._generate_empty_tracks(g_size=self.g_size)._fields.keys())
        all_frame_outputs = []
        for frame_index, (frame, gt, proposals) in enumerate(zip(frames, data['gt_instances'], data['proposals'])):
            ...
            if track_instances is None:
                track_instances = self._generate_empty_tracks(proposals, g_size=self.g_size)
            else:
                track_instances = Instances.cat([
                    self._generate_empty_tracks(proposals, g_size=self.g_size),
                    track_instances])
            ...
            if self.use_checkpoint and frame_index < len(frames) - 1:
                def fn(frame, gtboxes, *args):
                    frame = nested_tensor_from_tensor_list([frame], size_divisibility=32)
            ...
            all_frame_outputs.append(frame_res)

        if not self.training:
            outputs['track_instances'] = track_instances
        else:
            outputs['losses_dict'] = self.criterion.losses_dict
            outputs['all_frame_outputs'] = all_frame_outputs
        return outputs
...
def build(args):
    ...
    if args.memory_bank_type is not None and len(args.memory_bank_type) > 0:
        ...
        memory_bank = None
    # Define losses for per-frame processing (don't include unitrack here)
    losses = ['labels', 'boxes']
    
    # Flag comes from command-line args
    enable_unitrack_loss = getattr(args, 'enable_unitrack_loss', False)
    
    # Add weights for Unitrack loss components if enabled
    if enable_unitrack_loss:
        # Use only the total loss to avoid double-scaling
        weight_dict['loss_unitrack'] = getattr(args, 'unitrack_loss_coef', 1.0)
        # Individual components are returned for monitoring but not weighted separately
        # since they're already combined with alpha weights in Unitrackrion
    
    criterion = ClipMatcher(num_classes, matcher=img_matcher, weight_dict=weight_dict,
                          losses=losses, g_size=g_size, enable_unitrack_loss=enable_unitrack_loss)
    criterion.to(device)
    postprocessors = {}
    model = MOTR(
        backbone,
